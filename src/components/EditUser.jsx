...
import { insertUser,updateData, loggedUser } from "../redux/actions";
...
export default function EditUser({switcher, setswitcher, selectedUser}) {
  ...
  useEffect(() => {
    if (isValidated) {
      ...
      users.map((user,index)=>
        user.id == selectedUser.id ? store.dispatch(loggedUser({
          firstName: fName,
          lastName: lName,
          email: email,
          profilePic:selectedFile,
